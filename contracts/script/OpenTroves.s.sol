...
import {ERC20Faucet} from "../src/test/ERC20Faucet.sol";
...
contract OpenTroves is Script {
    ...
    function run() external {
        ...
        IHintHelpers hintHelpers = IHintHelpers(vm.envAddress("HINT_HELPER"));
        ...
        for (uint256 branch = 1; branch < 2; ++branch) {
            ...
            c.troveManager = ITroveManager(vm.envAddress("MANAGER"));
            ...
            if (c.borrowerOperations.getInterestBatchManager(vm.envAddress("BATCH_MANAGER")).maxInterestRate == 0) {
            ...
            for (uint256 i = 1; i <= 3; ++i) {
                ...
                if (i % 2 == 0) {
                    interestRate = c.troveManager.getLatestBatchData(vm.envAddress("BATCH_MANAGER")).annualInterestRate;
                    ...
                    c.borrowerOperations.setInterestBatchManager({
                        _troveId: troveId,
                        _newBatchManager: vm.envAddress("BATCH_MANAGER"),
                        _upperHint: upperHint,
                        _lowerHint: lowerHint,
                        _maxUpfrontFee: type(uint256).max // we don't care about fee slippage
                    });
