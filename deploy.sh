...
run_script() {
    ...
    if [ -z "$BATCH_MANAGER" ]; then
        ...
        forge_output=$(forge create --rpc-url "$RPC_URL" \
            --constructor-args "$STRATEGY_EOA" "$MANAGER" "$BORROWER_OPERATIONS" "$BOLD_TOKEN" "$WETH_PRICE_FEED" "$SORTED_TROVES" "$MIN_INTEREST_RATE" "$MAX_INTEREST_RATE" "$CURRENT_RATE" "$ANNUAL_MANAGEMENT_FEE" "$MIN_INTEREST_RATE_CHANGE_PERIOD" "$DISCOUNT_RATE_BATCH_MANAGER" \
